...
export function validateContent(path, data, sourceIDs = []) {
  ...
  if (path.startsWith("content/products/")) {
    ...
    for (const key of informationFields) {
      if (typeof data[key] !== "string" || typeof data.ka[key] !== "string")
        fail("Product information fields must exist in both languages.");
      if (!!data[key].trim() !== !!data.ka[key].trim())
        fail("Complete each product information field in both languages.");
    }
