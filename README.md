...
Required server environment: `CMS_ADMIN_USERNAME`, `CMS_PASSWORD_HASH`, `CMS_SESSION_SECRET`, `CMS_GITHUB_APP_ID`, `CMS_GITHUB_PRIVATE_KEY_BASE64`, `CMS_GITHUB_INSTALLATION_ID`, `CMS_REPOSITORY_ID`, plus `KV_REST_API_URL` and `KV_REST_API_TOKEN` (or the equivalent `UPSTASH_REDIS_REST_URL`/`UPSTASH_REDIS_REST_TOKEN`). Install the private GitHub App only on `untold13/fortis-pharmaceuticals`; its server-side installation tokens are restricted to that repository with contents-write permission. Missing configuration fails closed.

Sessions use random opaque IDs in HttpOnly, Secure, SameSite=Strict cookies, expire after eight hours and are stored by hashed ID in Redis. Logout deletes the server session; rotating the password hash or session secret invalidates previous sessions. Redis enforces atomic per-IP and global login limits. All writes require the fixed production origin and JSON content type. Credentials never enter the browser bundle or responses.
